...
def _inverted_res_block(x,
                        input_c,  # input channel
                        kernel_size,  # kernel size
                        exp_c,  # expanded channel
                        out_c,  # out channel
                        use_se,  # whether using SE
                        activation,  # RE or HS
                        stride,
                        block_id,
                        alpha: float = 1.0):
    bn = partial(layers.BatchNormalization, epsilon=0.0001, momentum=0.99)
...
def mobilenet_v3_small(input_shape=(224, 224, 3),
    ...
    if include_top is True:
        ...
        x = layers.Conv2D(filters=last_point_c,
                          ...
                          padding="same",
                          name='Conv_2')(x)
        x = HardSwish(name="Conv2/HardSwish")(x)
        # fc2
...
# mobilenetv2 = mobilenet_v3_large(num_classes=5)
